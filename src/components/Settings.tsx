...
export const Settings = () => {
    ...
    const getUser = async() => {
        const response = await axios.get("http://localhost/controle-fitness/backend/endpoints/getuser.php", {
            params: { id: id}
        });
    ...
    const addPhoto = async(id: any) => {
        ...
        if(fileUnput.current?.files && fileUnput.current.files.length > 0) {
            ...
            const response = await axios.post("http://localhost/controle-fitness/backend/endpoints/addPhoto.php",
                formData, {
                    headers: {
    ...
    const removePhoto = async(id: any) => {
        const response = await axios.post("http://localhost/controle-fitness/backend/endpoints/removePhoto.php", {
            id: id
        });
    ...
    const handleUpdateUser = async() => {
        const response = await axios.put("http://localhost/controle-fitness/backend/endpoints/updateUser.php", {
            id: id,
            name: inputName,
            email: inputEmail
        });
            ...
            <div className="bg-gray-200 border border-gray-300 rounded-lg p-5">
                ...
                <div className="flex gap-3 mt-5 items-center">
                    <div className="w-20 h-20 bg-yellow-default rounded-full">
                        <img 
                            src={`${userUpdate?.image_url ? `http://localhost/controle-fitness/backend/uploads/${userUpdate?.image_url}` : 'http://localhost/controle-fitness/backend/uploads/avatar.jpg'}`} 
                            alt="foto-perfil" 
