...
export const Task = () => {
  ...
  const getAllTask = async (userId: number) => {
    const response = await axios.get(
      `https://fitnessexclusive.com.br/api/controle-fitness-back/endpoints/tasks/getAllTask.php?userId=${userId}`
    );
  ...
  const delTask = async (id: number) => {
    const response = await axios.delete(
      "https://fitnessexclusive.com.br/api/controle-fitness-back/endpoints/tasks/delTask.php",
      {
        data: { id },
      }
    );
  ...
  const completedTask = async (id: number) => {
    const response = await axios.put(
      "https://fitnessexclusive.com.br/api/controle-fitness-back/endpoints/tasks/completedTask.php",
      { id }
    );
    if (response.data["result"]) {
      getAllTask(Number(userId));
  ...
  const handleBtnCreateTask = () => setOpenCreateTask(true);
  const handleBtnCloseCreatetask = () => setOpenCreateTask(false);

  const handleBtnUpdateTask = async (id: number) => {
    const response = await axios.get(
      "https://fitnessexclusive.com.br/api/controle-fitness-back/endpoints/tasks/getTask.php",
      { params: { id } }
    );
    if (response.data["result"]) {
  ...
  const handleBtnCloseUpdateTask = () => setOpenUpdateTask(false);

  const getComments = async (id: number) => {
    const response = await axios.get(
      "https://fitnessexclusive.com.br/api/controle-fitness-back/endpoints/tasks/getComments.php",
      { params: { id } }
    );
    if (response.data["result"]) {
  ...
  const handleBtnAddComment = async (id: number) => {
    if (inputComment) {
      const response = await axios.post(
        "https://fitnessexclusive.com.br/api/controle-fitness-back/endpoints/tasks/addComment.php",
        {
          task_id: id,
          label: inputComment,
        }
  ...
  return (
    <div className={`p-5 ${openCreateTask ? "cursor-not-allowed bg-black/60" : ""}`}>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 relative">
        {infoTasks.map((item) => (
          <div
            key={item.id}
            className="bg-dark-gray rounded-xl border-2 border-yellow-default py-5 flex flex-col items-center gap-2"
          >
      ...
      {openCreateTask && (
        <ModalCreateTask
          closeModalCreateTask={handleBtnCloseCreatetask}
          reloadTasks={() => getAllTask(Number(userId))}
        />
      )}

      {openUpdateTask && (
        <ModalUpdateTask
          taskValue={taskUpdate}
          closeModalUpdateTask={handleBtnCloseUpdateTask}
          reloadTasks={() => getAllTask(Number(userId))}
        />
      )}

      {!tasks && <p>Adicione sua primeira tarefa</p>}
      ...
      {tasks && (
        <ul className="space-y-3">
          {tasks.map((item) => {
            ...
            const isOverdue = dueDate < today && item.status !== "concluida";
            const isDueToday =
              dueDate.toDateString() === today.toDateString() && item.status !== "concluida";

            return (
              <li
                key={item.id}
                className={`border border-gray-300 rounded-lg p-4
                  ${item.status === "concluida" ? "bg-green-100" : "bg-gray-200"}
              ...
              >
                <div className="flex justify-between">
                  <h2
                    ...
                    } text-xl font-bold text-black/70`}
                  >
                    {item.title}
                  </h2>
                    ...
                    <button
                      ...
                      className={`px-2 rounded-md ${
                        item.status === "concluida"
                          ...
                          : "bg-yellow-default text-black border border-yellow-default font-bold hover:bg-yellow-default/70 cursor-pointer"
                      }`}
                    >
                ...
                <div className="flex justify-between text-sm text-black/50">
                  ...
                  {item.status === "concluida" ? (
                    <p>Concluído: {brazilianFormat(item.completed_at)}</p>
                  ) : (
                ...
                {(isDueToday || isOverdue) && (
                  <div>
                    {isDueToday && <p className="text-red-500 font-bold">VENCIMENTO HOJE</p>}
                    {isOverdue && <p className="text-red-500 font-bold">JÁ VENCEU</p>}
                  </div>
                )}
                  ...
                  <div className="border-t border-black/30">
                    <ul className="my-2 space-y-2">
                      {comments?.map((comment) => (
                        ...
                        >
                          <p>{brazilianFormat(comment.created_at)}</p>
                          <p>{comment.label}</p>
                        </li>
                      ))}
                    </ul>
